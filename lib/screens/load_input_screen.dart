...
class _LoadInputScreenState extends ConsumerState<LoadInputScreen> {
  ...
  void _addDetailedLoad() {
    if (_detailedFormKey.currentState!.validate()) {
      final power = InputParser.doubleOrNull(_powerValueController.text);
      final hours = InputParser.doubleOrNull(_dailyHoursController.text);
      if (power == null || power <= 0 || hours == null || hours < 0 || hours > 24) {
        _showInputError('أدخل قدرة وساعات صالحة ضمن الحدود المسموحة.');
        return;
      }
  ...
  void _addQuickLoad() {
    if (_quickFormKey.currentState!.validate()) {
      final power = InputParser.doubleOrNull(_quickPowerController.text);
      final hours = InputParser.doubleOrNull(_quickHoursController.text);
      if (power == null || power <= 0 || hours == null || hours < 0 || hours > 24) {
        _showInputError('أدخل قدرة وساعات صالحة ضمن الحدود المسموحة.');
        return;
      }
          ...
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    final power = InputParser.doubleOrNull(powerCtrl.text);
                    final hours = InputParser.doubleOrNull(hoursCtrl.text);
                    ...
                    try {
                      ...
                      _showInputError(error.toString());
                    }
                  },
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      keyboardType: TextInputType.number,
                      onChanged: (value) {
                        final val = InputParser.doubleOrNull(value);
                        if (val != null) {
